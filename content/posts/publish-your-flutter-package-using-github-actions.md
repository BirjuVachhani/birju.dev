...
Click on `Add a new secret` button to add a **secret**. Here's all the secrets that we're going to need.
...
Use this **workflow** only if your package is a **Dart package**, not a **Flutter package**. For Flutter packages, use the following workflow:
...
* `google/dart:latest` container image will provide us with the lastest **dart installation**.
